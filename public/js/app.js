...
async function showCart() {
    ...
    xhttp.onreadystatechange = async function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            document.getElementById('homePageContent').innerHTML += containerForData;
            let noOfBooks = jo.arrList.length;
            let userId = jo.userId;
            let booksList = []
            for (let i = 0; i < noOfBooks; i++) {
                let res = await fetch(`https://www.googleapis.com/books/v1/volumes/${jo.arrList[i]}`);
                let data = await res.json();
                booksList.push(data.id);
                let dataForArray = `
                <div class="blog-box row">
                    <div class="col-md-4">
                        <div class="post-media">
            ...
            document.getElementById('searchDataCon').innerHTML += `
                <p>Tariff:</p>
                <p>&#8377;10 per book, per day</p>
                <h4>For how many days do you want to rent the books for?</h4>
                <input type="text" id="noDays" onkeyup="calcAmt(${noOfBooks})">
                <h4><strong>The total amount would be</strong></h4>&#8377;<input type="text" id="totalAmount" readonly="readonly">
                <!--<button id="addOrder" onclick="addOrderHistory(${userId}, ${noOfBooks}, ${booksList})" class="btn btn-primary" style="margin-left: 5px">Rent</button>-->
                <button id="addOrder" onclick="addOrderHistory('${userId}', '${noOfBooks}', '${booksList}')" class="btn btn-primary" style="margin-left: 5px">Rent</button>
            `;
...
function calcAmt(noOfBooks) {
    let noOfDays = document.getElementById('noDays').value;
    let total = 10 * noOfBooks * noOfDays;
    document.getElementById('totalAmount').value = total;
}

async function addOrderHistory(userId, noOfBooks, booksList) {
    let noOfDays = document.getElementById('noDays').value;
    let totalVal = document.getElementById('totalAmount').value;
    if (noOfDays == 0 || totalVal == 0) {
        alert("Please enter no. of Days");
    }
    console.log(typeof(userId) + "    " + typeof(booksList))
...
async function showHistory() {
    let userIdHid = document.getElementById('userIdHid').value;
    fetch('/order/showHistory/' + userIdHid)
        .then(
            (res) => {
                document.getElementById('homePageContent').innerHTML = "";
                let header = `
            <div class="page-title wb">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-8 col-md-8 col-sm-12 col-xs-12">
                            <h2><i class="fa fa-leaf bg-green"></i> Your Rental History</h2>
                        </div><!-- end col -->
                        <div class="col-lg-4 col-md-4 col-sm-12 hidden-xs-down hidden-sm-down">
                            <ol class="breadcrumb">
                                <li class="breadcrumb-item"><a href="#">Home</a></li>
                                <li class="breadcrumb-item"><a href="#">Your Rental History</a></li>
                            </ol>
                        </div><!-- end col -->                    
                    </div><!-- end row -->
                </div><!-- end container -->
            ...
            <section class="section wb">
                <div class="container">
                    <div class="row">
                        <div class="col-lg-9 col-md-12 col-sm-12 col-xs-12">
                            <div class="page-wrapper">
                                <div class="blog-list clearfix" id="searchDataCon" style="width: 1500px;">
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                res.json().then(async function(data) {
                    for (let i = 0; i < data.length; i++) {
                        let dataForArray = `
                            <div id= "${data[i]._id}" class="blog-meta big-meta col-md-8" style="height: 350px; overflow-y: scroll;">
                                <h5>Rental ID: &nbsp; ${data[i]._id}</h5><h6>No of Days: &nbsp; ${data[i].noOfDays}</h6><h6>Total Amount: &nbsp; &#8377;${data[i].totalAmount}</h6>
                            </div><br><br><br>
                        `;
                        document.getElementById('searchDataCon').innerHTML += dataForArray;
                    }
                    for (let i = 0; i < data.length; i++) {
                        let booksData = data[i].books[0].split(',');
                        for (let j = 0; j < booksData.length; j++) {
                            let res = await fetch(`https://www.googleapis.com/books/v1/volumes/${booksData[j]}`);
                            ...
                            <div class="blog-box row">
                                <div class="col-md-4">
                                    <div class="post-media">
                                        <a href="garden-single.html" title="">
                                            <img src="${dataApi.volumeInfo.imageLinks.thumbnail}" alt="" class="img-fluid">
                                            <div class="hovereffect"></div>
                                        </a>
                                    </div><!-- end media -->
                                </div><!-- end col -->
                                ...
                                <div class="blog-meta big-meta col-md-8">
                                    <span class="bg-aqua"><a href="garden-category.html" title="">${dataApi.volumeInfo.categories}</a></span>
                                    <span class="bg-aqua" onclick="addToFavourites('${dataApi.id}')" id="${dataApi.id}><i class="fa fa-search"></i>Add to Favourites</span> 
                                    <span class="bg-aqua" onclick="delFromFavourites('${dataApi.id}')" id="${dataApi.id}><i class="fa fa-heart"></i>Delete from Favourites</span> <br>
                                    <h4><a href="garden-single.html" title="">${dataApi.volumeInfo.title}: ${dataApi.volumeInfo.subtitle}</a></h4>
                                    <div style="height: 200px; overflow-y: scroll;">
                                    <p>${dataApi.volumeInfo.description}</p>
                                    </div>
                                    <small><a href="garden-category.html" title=""><i class="fa fa-eye"></i> ${dataApi.volumeInfo.publisher}</a></small>
                                    <small><a href="garden-single.html" title="">${dataApi.volumeInfo.publishedDate}</a></small>
                                    <small><a href="#" title="">${dataApi.volumeInfo.pageCount}</a></small>
                                </div><!-- end meta -->
                            </div>
                            <br>`;
                            document.getElementById(data[i]._id).innerHTML += dataForArray;
                        }
                    }
                });
            }
